...
const MATERIALS = {
  grass: `
    ...
    void surf(vec2 uv, out vec3 albedo, out float height, out float roughness, out float ao) {
      ...
      albedo = mix(vec3(0.035, 0.075, 0.025), vec3(0.16, 0.27, 0.065), macro);
      albedo = mix(albedo, vec3(0.27, 0.23, 0.09), dead * 0.24);
  ...
  dirt: `
    ...
    void surf(vec2 uv, out vec3 albedo, out float height, out float roughness, out float ao) {
      ...
      albedo = mix(vec3(0.09, 0.047, 0.021), vec3(0.32, 0.16, 0.06), strata);
  ...
  mud: `
    void surf(vec2 uv, out vec3 albedo, out float height, out float roughness, out float ao) {
      ...
      albedo = mix(vec3(0.045, 0.028, 0.016), vec3(0.17, 0.085, 0.035), relief);
  ...
  gravel: `
    void surf(vec2 uv, out vec3 albedo, out float height, out float roughness, out float ao) {
      ...
      albedo = mix(vec3(0.13, 0.12, 0.095), vec3(0.36, 0.31, 0.22), medium);
  ...
  rock: `
    void surf(vec2 uv, out vec3 albedo, out float height, out float roughness, out float ao) {
      ...
      albedo = mix(vec3(0.10, 0.12, 0.10), vec3(0.30, 0.30, 0.23), strata);
